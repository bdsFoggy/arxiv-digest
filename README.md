...
首次安装默认补查 7 天。后续根据上次成功周期起点回溯，并保留 2 天重叠窗口。OAI 在论文公告后更新，长时间超出重叠窗口的元数据发布延迟仍可能需要人工扩大补查范围。每日 RSS 不用作历史补查。
...
相关代码推送到 `main` 后自动运行测试和 `preview`。定时运行仍执行正常日报：新加坡时间 08:07、10:17、14:27、20:37；当天已完成则跳过。GitHub 可能延迟触发。
